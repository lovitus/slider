...
#### Setup a redirect proxy and a dns server with slider
slider.conf
```bash
verbose=True
listen=redir://:1081
...
1. all dns requests for domain example1.com will be forward to slider(:5353) by dnsmasq
2. slider will forward dns requests to 8.8.8.8:53 in tcp via forwarders
3. the resolved ip address will be added to ipset "myset" by dnsmasq
4. all tcp requests to example1.com will be redirect to slider(:1081) by iptables
5. slider then forward requests to example1.com via forwarders
